...
// add %g1,4,%g3(g1 + 4 -> g3)
func InitSparc64(asmsh *as.AsmShell, bigEndian bool) {
    ...
    if bigEndian {
        ...
        asmsh.KeystoneMode = keystone.MODE_SPARC64 + keystone.MODE_BIG_ENDIAN
        asmsh.UnicornMode = uc.MODE_SPARC64 | uc.MODE_BIG_ENDIAN
        asmsh.Prompt = "(sparc64)> "
    } else {
        // keystone unsupported
        asmsh.UnicornArch = uc.ARCH_MAX
        asmsh.KeystoneMode = keystone.MODE_SPARC64 + keystone.MODE_LITTLE_ENDIAN
        asmsh.UnicornMode = uc.MODE_SPARC64 | uc.MODE_LITTLE_ENDIAN
        asmsh.Prompt = "(sparc64el)> "
    }
    asmsh.SavedCtx = nil
