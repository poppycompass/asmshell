...
func (asmsh *AsmShell) PrintCtx(c *ishell.Context, mu uc.Unicorn, mnemonic string, code []byte) error {
...
func (asmsh *AsmShell) Run(c *ishell.Context, mnemonic string, code []byte) error {
    ...
    err = mu.MemWrite(esp - (asmsh.SavedStackSize / 2), asmsh.SavedStack)
    if err != nil {
        return err
    }
    ...
    asmsh.SavedStack, err  = mu.MemRead(esp - (asmsh.SavedStackSize / 2), asmsh.SavedStackSize)
    if err != nil {
        return err
    }
